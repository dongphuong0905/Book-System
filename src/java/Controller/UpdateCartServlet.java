    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for(Cookie cookie : cookies){
            if(cookie.getName().equals("shoppingCart")){
                itemList = new Gson().fromJson(cookie.getValue(), new TypeToken<List<Item>>(){}.getType());
            }
        }
        int i = 0;
        for(String s : quantity){
            itemList.get(i++).setQuantity(Integer.parseInt(s));
        }
        Cookie cookie = new Cookie("shoppingCart", new Gson().toJson(itemList));
